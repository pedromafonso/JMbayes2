jm <- function (Surv_object, Mixed_objects, time_var, recurrent = FALSE,
    ...
    Data <- list(n = nY, idL = idL, idL_lp = idL_lp, unq_idL = unq_idL,
                 ...
                 recurrent = !isFALSE(recurrent), which_term_h = which(strata == 2), ##!! new #?? recurrent is probably better in the model info
    ...
    frailty <- rep(0.5, nT) #!! new #?? not sure what would be the ideal initial
    alphaF <- 0.0 #!! new
    ...
    initial_values <- list(betas = betas, log_sigmas = log_sigmas,
                           ...
                           alphaF = alphaF, frailty = frailty, sigmaF = sigmaF) #!! new
    ############################################################################
    ############################################################################
    # Limits
    ...
    prs <- list(mean_betas_HC = mean_betas_HC, Tau_betas_HC = Tau_betas_HC,
                ...
                mean_alphaF = 0.0, Tau_alphaF = diag(1), #!! new
                gamma_prior_sigmaF = TRUE, #!! new
                sigmaF_df = 3.0, #!! new
                sigmaF_sigmas = 5.0, #!! new
                sigmaF_shape = 5.0, #!! new 
                sigmaF_mean = 0.1 #!! new #?? not sure if this is the value that we need, for the other sigmas we use exp(log_sigmas)
                )
