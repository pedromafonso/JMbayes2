...
List mcmc_cpp (List model_data, List model_info, List initial_values,
  ...
  double alphaF = as<double>(initial_values["alphaF"]); //!! new
  ...
  mat res_frailty(n_iter, frailty.n_elem, fill::zeros); //!! new //?? not sure if this is the right dimension
  ...
  for (uword it = 0; it < n_iter; ++it) {
    ...
    if (recurrent) { //!! new
      // update frailty
      res_frailty.row(it) = frailty;
      acceptance_frailty.row(it) = frailty.ones();
      // update alphaF
  ...
  return List::create(
    Named("mcmc") = List::create(
      ...
      Named("frailty") = res_frailty  //!! new //?? probably we don't want to save them all
    ),
    Named("acc_rate") = List::create(
      ...
      Named("frailty") = acceptance_frailty //!! new //?? why we do not use mean() here check the acceptance_b
    ),
...
arma::vec logLik_jm (List thetas, List model_data, List model_info,
  ...
  uvec id_H_fast = create_fast_ind(id_H + 1);
  uvec id_h_fast = create_fast_ind(id_h + 1);
  double alphaF = as<double>(thetas["alphaF"]); //!! new
...
arma::mat mlogLik_jm (List res_thetas, arma::mat mean_b_mat, arma::cube post_vars,
  ...
  uvec id_H_fast = create_fast_ind(id_H + 1);
  uvec id_h_fast = create_fast_ind(id_h + 1);
  double alphaF = as<double>(res_thetas["alphaF"]); //!! new
  vec frailty = as<vec>(res_thetas["frailty"]); //!! new
  uvec which_term_h = as<uvec>(model_data["which_term_h"]) - 1; //!! new
  uvec which_term_H = as<uvec>(model_data["which_term_H"]) - 1; //!! new
  /////////////
  mat out(n, K);
...
arma::cube simulate_REs (List Data, List MCMC, List control) {
  ...
  //////////////////////////////
  uvec which_term_h = as<uvec>(Data["which_term_h"]) - 1; //!! new
  uvec which_term_H = as<uvec>(Data["which_term_H"]) - 1; //!! new
  ...
  mat frailty = trans(as<mat>(MCMC["frailty"])); //!! new
  vec alphaF = as<vec>(MCMC["alphaF"]); //!! new
  uword K = D.n_slices;
  uword q = D.slice(0).n_rows;
  mat sds(q, K);
  cube L(q, q, K);
  ...
  for (uword it = 0; it < n_samples; ++it) {
    ...
    double alphaF_it = alphaF.at(it); //!! new //?? not sure if it alphaF is a vector or not
    vec frailty_it = frailty.col(it); //!! new
    ///////////////////////
    ...
    vec alphaF_H = WH_gammas.ones(); //!! new
    vec alphaF_h = Wh_gammas.ones(); //!! new
    alphaF_H.rows(which_term_h).fill(alphaF_it); //!! new
    alphaF_h.rows(which_term_H).fill(alphaF_it); //!! new
    vec frailty_H = WH_gammas.zeros(); //!! new
    vec frailty_h = Wh_gammas.zeros();; //!! new
    frailty_h = frailty_it.rows(id_h); //!! new
    frailty_H = frailty_it.rows(id_H_); //!! new
    vec logLik_surv =
      log_surv(W0H_bs_gammas, W0h_bs_gammas, W0H2_bs_gammas,
               WH_gammas, Wh_gammas, WH2_gammas,
               WlongH_alphas, Wlongh_alphas, WlongH2_alphas,
